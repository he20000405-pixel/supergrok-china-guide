...
![Grok 登录状态确认页面：先确认账号已登录再获取 Grok User ID](img/grok-login-homepage-hd.png)
2. 在同一个浏览器里打开：`https://grok.com/api/auth/session`。
![Grok User ID 获取步骤：在浏览器地址栏输入 grok.com/api/auth/session](img/grok-user-id-session-url-hd.png)
3. 如果已经登录，会看到一段 JSON 内容。
4. 找到 `userId` 字段，复制引号里的完整内容。
![Grok User ID JSON 示例：从 grok.com/api/auth/session 返回内容中复制 userId 字段](img/grok-user-id-json-example-hd.png)
...
![chonggrok.com SuperGrok 充值页面：选择套餐前确认不需要密码、只提交 Grok User ID](img/chonggrok-supergrok-home-hero-hd.png)
2. 选择 1 / 2 / 3 个月套餐。
![chonggrok.com SuperGrok 套餐价格示例：1个月、2个月、3个月充值周期对比](img/chonggrok-supergrok-pricing-plans-hd.png)
3. 使用支付宝或微信完成付款，保存订单号和卡密。
4. 打开 [https://chonggrok.com/verify](https://chonggrok.com/verify)，输入卡密。
![chonggrok.com SuperGrok 卡密核销页面：输入卡密后按步骤提交 Grok User ID](img/chonggrok-supergrok-redeem-console-hd.png)
5. 按页面提示提交 Grok User ID。
6. 等待处理完成后，回到 grok.com 检查订阅状态和权益。
